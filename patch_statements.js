...
let content = fs.readFileSync('electron/ipc/statements.ts', 'utf8');

const newMethod = `
  ipcMain.handle('statement:get', async (_, accountType: 'party' | 'fund', accountId: number, currency: 'IQD' | 'USD', fromDate?: string, toDate?: string) => {
    const database = await db.getDb();
    
    let dateFilterParty = '';
    let dateFilterFundTreasury = '';
    let dateFilterFundJournal = '';
    const paramsParty: any[] = [accountId, currency];
    const paramsFundTreasury: any[] = [currency, currency, currency, currency, accountId];
    const paramsFundJournal: any[] = [accountId];
    ...
    if (accountType === 'party') {
      const rows = await database.all(\`
        SELECT 
          ...
          CASE 
            ...
            WHEN pt.type = 'journal' THEN 'سند قيد يومية'
            ELSE pt.type
      ...
      // Calculate Previous Balance if fromDate is provided
      let previousBalanceRow = null;
      if (fromDate) {
        const prevTx = await database.get(\`
        ...
        if (prevTx) {
          ...
          // Check if party has opening balance that was applied?
          // The opening balance is typically inserted as the first transaction, so it would be caught if date < fromDate.
        }
      ...
      if (previousBalanceRow) {
        return [previousBalanceRow, ...rows];
      }
      return rows;
    } else if (accountType === 'fund') {
      const fund = await database.get('SELECT opening_balance_iqd, opening_balance_usd FROM funds WHERE id = ?', accountId);
      
      // 1. Calculate Previous Balance up to fromDate
      let previousBalance = (currency === 'IQD' ? fund.opening_balance_iqd : fund.opening_balance_usd) || 0;
      
      if (fromDate) {
        const prevTreasury = await database.all(\`
        ...
        const prevJournal = await database.all(\`
      ...
      const treasuryRows = await database.all(\`
        SELECT 
          date,
          'treasury' as type,
      ...
      const journalRows = await database.all(\`
        SELECT 
          jv.date,
          'journal' as type,
          'سند قيد يومية' as movement_type,
      ...
      let previousBalanceRow = null;
      if (fromDate && (previousBalance !== 0 || statementRows.length > 0)) {
        previousBalanceRow = {
          date: fromDate,
          operation_type: 'opening',
          movement_type: 'رصيد مدور',
        ...
        return [previousBalanceRow, ...statementRows];
      }
...
content = content.replace(
  /ipcMain\.handle\('statement:get'[\s\S]*?\}\);\n/m,
  newMethod + '\n'
);

fs.writeFileSync('electron/ipc/statements.ts', content);
console.log('Backend statements.ts Patched');
